...
{
    [Activity(Label = "WeatherAct", Theme = "@style/Theme.NoTitle")]
    public class WeatherAct : Activity
    {
        ...
        protected override void OnCreate(Bundle bundle)
        {
            ...
            SetContentView(Resource.Layout.weatherCheckLayout);
            InitializeControl();
        ...
        async Task<bool> LocationSelected()
        {
            ...
            try
            {
                ...
                {
                    objYahooWeatherClass = Newtonsoft.Json.JsonConvert.DeserializeObject<WeatherClass>(strWeatherJson);
        ...
        void fnAlertMsg(string strTitle, string strMsg, string strOk, string strCancel, Context context)
        {
            ...
            alertMsg.SetButton2(strOk, delegate (object sender, DialogClickEventArgs e)
            {
                if (e.Which == -2)
                {
                    ...
                    //  fnAlertMsgOne(GetString(Resource.String.ApplicationName), "Try again now", "OK", this);
                    Toast.MakeText(this,"Try again now", ToastLength.Short).Show();
                }
            });
